...
--
-- Hôte : localhost
-- Généré le :  ven. 04 juin 2021 à 16:03
...
INSERT INTO `editeur` (`id`, `nom`, `information`, `date_creation`, `utilisateur_id`, `image`) VALUES
(1, 'Éditions Emmanuelle Collas', 'Aucune information supplémentaire', '2018-01-01', 1, ''),
(2, 'Le Cherche midi', 'Maison d`édition française fondée en 1978, les éditions Le Cherche midi publient des ouvrages de littérature française et étrangère, de la poésie, des œuvres dans le domaine de l`humour, des documents, des beaux-livres et livres pratiques. La maison d`édition collabore avec de nombreuses associations comme Amnesty International, la Ligue des droits de l`homme, la Licra ou encore la Fondation Abbé Pierre.', '1978-06-01', 1, 'lecherchemidi.jpg'),
(3, 'Seuil', 'Créées par Henri Sjöberg en 1935, les Éditions du Seuil font partie des plus grandes maisons d`éditions françaises et sont présentes dans tous les domaines éditoriaux : littératures française et étrangère, thrillers et policiers, sciences humaines, documents, spiritualités, sciences, jeunesse et beaux-livres.', '1935-01-01', 1, ''),
(4, 'J`ai Lu', 'A la demande d`Henri Flammarion en 1958, Frédéric Ditis créé la maison d`édition française J`ai lu, qui publie principalement en format poche. Sa ligne éditoriale est variée, allant de la littérature générale à la science-fiction, en passant par le roman policier et le roman d`amour. Les éditions J`ai lu publient chaque année plus de 400 nouveautés au format poche.', '1958-04-09', 1, ''),
(5, 'Le Masque', 'Fondé en 1927 par Albert Pigasse, Le Masque est une maison d’édition française qui se consacre exclusivement à la littérature noire, et est notamment l`éditeur historique d’Agatha Christie. Les éditions du Masque proposent également plusieurs collections : les Grands Formats, pour les auteurs de demain, la collection Labyrinthes, en format poche, et MsK, pour les jeunes lecteurs.', '1925-01-01', 2, ''),
(10, 'Andre', 'editeur test 2.1', '2021-05-31', 3, ''),
(13, 'Andrea', 'Votre éditeur ici et la', '2021-06-07', 1, 'vector-male-student-icon-design.jpg');
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `livre_after_update` AFTER UPDATE ON `livre` FOR EACH ROW BEGIN
...
--
-- Structure de la table `livre_archive`
--
...
--
-- Déchargement des données de la table `livre_archive`
--

INSERT INTO `livre_archive` (`id`, `titre`, `nb_pages`, `rating`, `resumer`, `editeur_id`, `date_edition`, `courriels_auteur`, `genre`, `image`) VALUES
(31, 'L\'aurore 24', 1234, 3.94, 'ffsfsgsgdddufurtttssfdsfsd mod', 10, '2021-06-08', 'unepeanut@cmail.com', 'Documentaire', ''),
(36, 'La Vengeance des mères 2', 454, 3.81, 'Écrivez le ', 10, '2021-06-08', 'drago-jo@hotmail.com', 'Policier', ''),
(37, 'Furie modi', 1345, 4.43, 'Écrivez le résumer', 10, '2021-06-09', 'go-jo@hotmail.com', 'Fantastique', '');

-- --------------------------------------------------------

--
-- Structure de la table `livre_audit`
--

CREATE TABLE `livre_audit` (
  `id` int(11) NOT NULL,
  `livre_id` int(11) NOT NULL,
  `changetype` enum('Nouveau','Modifié','Effacé') CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL,
  `changetime` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

--
-- Déchargement des données de la table `livre_audit`
--

INSERT INTO `livre_audit` (`id`, `livre_id`, `changetype`) VALUES
(18, 38, 'Nouveau'),
(19, 38, 'Modifié'),
(20, 38, 'Modifié'),
(21, 39, 'Nouveau'),
...
--
-- Structure de la table `livre_audit_archive`
--

CREATE TABLE `livre_audit_archive` (
  `id` int(11) NOT NULL,
  `livre_id` int(11) NOT NULL,
...
--
-- Déchargement des données de la table `livre_audit_archive`
--

INSERT INTO `livre_audit_archive` (`id`, `livre_id`, `changetype`) VALUES
(3, 31, 'Nouveau'),
(13, 36, 'Nouveau'),
(14, 36, 'Effacé'),
(15, 37, 'Nouveau'),
...
(1, 'Jonathan Lamontagne', 'admin', 'admin'),
(2, 'Mohammed Errabie', 'admin2', 'admin2'),
(3, 'Koffi Dzakou', 'admin3', 'admin3');
...
--
-- Index pour la table `livre_archive`
--
ALTER TABLE `livre_archive`
  ADD PRIMARY KEY (`id`),
  ADD KEY `ix_editeur_id` (`editeur_id`);

--
-- Index pour la table `livre_audit`
--
ALTER TABLE `livre_audit`
  ADD PRIMARY KEY (`id`),
  ADD KEY `ix_livre_id` (`livre_id`),
  ADD KEY `ix_changetype` (`changetype`),
  ADD KEY `ix_changetime` (`changetime`);

--
-- Index pour la table `livre_audit_archive`
--
ALTER TABLE `livre_audit_archive`
  ADD PRIMARY KEY (`id`),
  ADD KEY `ix_livre_id` (`livre_id`),
  ADD KEY `ix_changetype` (`changetype`),
  ADD KEY `ix_changetime` (`changetime`);
...
--
-- AUTO_INCREMENT pour la table `livre_archive`
--
ALTER TABLE `livre_archive`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=38;

--
-- AUTO_INCREMENT pour la table `livre_audit`
--
ALTER TABLE `livre_audit`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=31;

--
-- AUTO_INCREMENT pour la table `livre_audit_archive`
--
ALTER TABLE `livre_audit_archive`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=31;
...
--
ALTER TABLE `editeur`
  ADD CONSTRAINT `fk_utilisateur_editeur` FOREIGN KEY (`utilisateur_id`) REFERENCES `utilisateur` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;
...
--
-- Contraintes pour la table `livre_archive`
--
ALTER TABLE `livre_archive`
  ADD CONSTRAINT `FK_editeur_livre_archive` FOREIGN KEY (`editeur_id`) REFERENCES `editeur` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Contraintes pour la table `livre_audit`
--
ALTER TABLE `livre_audit`
  ADD CONSTRAINT `FK_audit_livre_id` FOREIGN KEY (`livre_id`) REFERENCES `livre` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Contraintes pour la table `livre_audit_archive`
--
ALTER TABLE `livre_audit_archive`
  ADD CONSTRAINT `FK_livre_audit_archive_livre_id` FOREIGN KEY (`livre_id`) REFERENCES `livre_archive` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;

DELIMITER $$
--
-- Évènements
--
CREATE DEFINER=`root`@`localhost` EVENT `archive_livre` ON SCHEDULE EVERY 1 MINUTE STARTS '2021-06-03 19:52:00' ON COMPLETION NOT PRESERVE ENABLE DO BEGIN
